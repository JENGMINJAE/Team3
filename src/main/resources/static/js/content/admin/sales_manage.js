...
function eventYear(){
    ...
    const select_div = document.querySelector('.year-chart');
    let str = '';
    
    mychart.destroy();
    fetch('/admin/drawTable', { //요청경로
        method: 'POST',
        cache: 'no-cache',
        headers: {
            'Content-Type': 'application/json; charset=UTF-8'
    ...
    .then((data) => {//data -> controller에서 리턴되는 데이터!
        ...
        data.forEach((e, i) => {
            str += `
                <tr>
                    <td>${i + 1}</td>
                    <td>${e.teacherVO.memberVO.operatorVOList[0].payDay}</td>
                    <td>${e.teacherVO.memberVO.memberName}</td>
                    <td>${e.className}</td>
                    <td></td>
                </tr>
            `;
        ...
        select_div.insertAdjacentHTML('afterbegin', str);
